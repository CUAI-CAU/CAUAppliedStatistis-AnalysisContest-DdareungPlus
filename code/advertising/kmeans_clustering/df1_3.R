...
df1_1_ed <- subset(df1_1, select=-c(usetime_2,usetime_3,usetime_4,usetime_5,age_20, age_30,age_40,age_50))
str(df1_1_ed)


# 파이썬에서 elbow 확인
df1_1_ed2 <- subset(df1_1_ed, select=-c(대여소번호))
write.csv(df1_1_ed2, file="df1_3_ed2.csv")


# 군집분석
kmeans <- kmeans(df1_1_ed2,5, nstart=10) 
...
df1_1_ed$kmeans <- factor(df1_1_ed$kmeans, levels=c(1,2,3,4,5), labels=c("one","two","three","four","five"))
