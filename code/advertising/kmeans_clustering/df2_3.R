...
as.factor(df1_1$대여소번호)
###################
# k-means 군집분석#
###################
# 휴일여부:2 , 대여시간:2
df1_1 <- subset(df, 대여시간 == 3 & 휴일여부 ==2)
...
df1_1$usetime_3_4_5 <- (df1_1$usetime_3+df1_1$usetime_4+ df1_1$usetime_5)/3
df1_1$age_20_30 <- (df1_1$age_20 + df1_1$age_30)/2
df1_1$age_40_50 <- (df1_1$age_40 + df1_1$age_50)/2
df1_1_ed <- subset(df1_1, select=-c(usetime_1,usetime_2,usetime_3,usetime_4,usetime_5,age_20, age_30,age_40, age_50))
str(df1_1_ed)


# 파이썬에서 elbow 확인
df1_1_ed2 <- subset(df1_1_ed, select=-c(대여소번호))
write.csv(df1_1_ed2, file="df2_3_ed2.csv")
