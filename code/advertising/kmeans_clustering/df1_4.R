...
df1_1$usetime_3_4 <- (df1_1$usetime_4 + df1_1$usetime_4)/2
df1_1$age_20_30 <- (df1_1$age_20 + df1_1$age_30)/2
df1_1$age_40_50 <- (df1_1$age_40 + df1_1$age_50)/2
df1_1_ed <- subset(df1_1, select=-c(usetime_3,usetime_4,age_20, age_30,age_40,age_50))
str(df1_1_ed)


# 파이썬에서 elbow 확인
df1_1_ed2 <- subset(df1_1_ed, select=-c(대여소번호))
write.csv(df1_1_ed2, file="df1_4_ed2.csv")
...
df1_1_ed <- df1_1_ed[!(df1_1_ed$kmeans ==3),]  # 이상치 제거
df1_1_ed$kmeans <- as.factor(df1_1_ed$kmeans)
df1_1_ed$kmeans <- factor(df1_1_ed$kmeans, levels=c(1,2,4), labels=c("one","two","three"))
